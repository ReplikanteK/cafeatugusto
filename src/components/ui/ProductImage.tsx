...
export function ProductImage({ src, alt, className="aspect-[4/3]" }: { src: string; alt: string; className?: string }) {
  ...
  return (
    ...
    <img src={src} alt={alt} onError={()=>setErr(true)} className="relative w-full h-full object-contain brightness-100 contrast-100 drop-shadow-[0_8px_16px_rgba(0,0,0,0.12)] mix-blend-normal" />
    </div>
  );
}
