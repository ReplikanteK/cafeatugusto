...
export default function ComparativasPage() {
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
      ...
      <p className="text-sm text-stone-400 mt-2">3 enfrentamientos curados spec-by-spec. Cada URL es landing SEO directa a compra.</p>
      <div className="grid md:grid-cols-3 gap-6 mt-8">
        {COMPARATIVES.map(c=> (
          <Link key={c.slug} href={`/comparativas/${c.slug}`} className="rounded-2xl bg-stone-900 border border-stone-800 p-6 hover:border-amber-500/40 transition-all flex flex-col">
            <span className="text-xs font-mono text-amber-400">{c.slug}</span>
