...
import { VersusCta, AccessoryCta } from "@/components/ui/VersusCta";
import { isRecommendableMachine, isRecommendableGrinder } from "@/lib/eligibility";
...
function findSeed(asin: string) {
  const m = MACHINES_SEED.find(x=> x.asin===asin);
  if (m) return { kind: "machine" as const, recommendable: isRecommendableMachine(m) };
  const g = GRINDERS_SEED.find(x=> x.asin===asin);
  if (g) return { kind: "grinder" as const, recommendable: isRecommendableGrinder(g) };
  return { kind: "unknown" as const, recommendable: false };
}
...
export default async function ComparativePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
      <p className="text-xs font-mono text-amber-400 tracking-widest">COMPARATIVA • PRECIOS DINÁMICOS DEL CATÁLOGO</p>
      ...
      <div className="grid md:grid-cols-2 gap-6 mt-8">
        {[c.a,c.b].map((p, idx)=> {
          ...
          <div key={p.asin} className="rounded-xl bg-stone-900 border border-stone-800 overflow-hidden">
            ...
            <div className="p-4"><p className="font-bold text-white">{p.name}</p><VersusCta asin={p.asin} name={p.name} slug={c.slug} role={idx===0 ? "a" : "b"} recommendable={seed.recommendable} /></div>
          </div>
        )})}
      </div>
      ...
      <div className="mt-8 rounded-2xl bg-stone-900 border border-stone-800 p-5">
        ...
        <div className="grid md:grid-cols-2 gap-3 mt-3">
          {c.accessories.map(a=> (
            <AccessoryCta key={a.asin} asin={a.asin} name={a.name} note={a.note} slug={c.slug} />
          ))}
        </div>
      </div>
