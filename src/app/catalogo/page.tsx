...
const allItems: Item[] = [
  ...MACHINES_SEED.map(m=> ({
    id: m.id, type:"machine" as const, name: m.model, brand: m.brand, price: m.priceApproxEUR, image: m.image, asin: m.asin,
    diam: m.specs.portafilterDiameter, pid: m.specs.pid, boiler: m.specs.boilerType, heat: m.specs.startupTimeSeconds, integrated: m.grinderIntegrated,
  })),
  ...GRINDERS_SEED.map(g=> ({
    id: g.id, type:"grinder" as const, name: g.model, brand: g.brand, price: g.priceApproxEUR, image: g.image, asin: g.asin,
    burr: g.specs.burrType, burrSize: g.specs.burrSizeMM, adj: g.specs.grindAdjustment, retention: g.performance.retention>=4 ? "Single-Dose" : g.specs.hopperCapacityGrams>100 ? "Con Tolva" : "Manual", focus: g.specs.espressoCapable && g.specs.filterCapable ? "Polivalente" : g.specs.espressoCapable ? "Espresso" : "Filtro",
  })),
];
...
export default function CatalogoPage() {
  ...
  const filtered = useMemo(()=> allItems.filter(i=>{
    ...
    return true;
  }), [cat, diam, pidOnly, noIntegrated, maxPrice]);
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="mt-6 flex flex-wrap gap-3 items-center border-y border-stone-800 py-4">
        <div className="flex gap-2">
          {(["all","machines","grinders"] as Cat[]).map(c=> <button key={c} onClick={()=>setCat(c)} className={`px-3 py-1.5 rounded-full text-xs font-bold border ${cat===c?"bg-amber-600 text-white border-amber-600":"bg-stone-900 text-stone-400 border-stone-800"}`}>{c==="all"?"Todas":c==="machines"?"Máquinas":"Molinillos"}</button>)}
        </div>
        <select value={diam} onChange={e=>setDiam(e.target.value as Diam)} className="px-3 py-1.5 rounded-lg bg-stone-900 border border-stone-800 text-xs">
          <option value="all">Diámetro — Cualquiera</option>
          <option value="51">51mm</option>
          <option value="54">54mm</option>
          <option value="57">57mm</option>
          <option value="58">58mm Pro</option>
        </select>
        <label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={pidOnly} onChange={e=>setPidOnly(e.target.checked)} /> Solo PID</label>
        <label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={noIntegrated} onChange={e=>setNoIntegrated(e.target.checked)} /> Sin molino integrado</label>
        <div className="flex items-center gap-2 text-xs">
          <span>≤{maxPrice}€</span>
          <input type="range" min={100} max={2000} step={50} value={maxPrice} onChange={e=>setMaxPrice(Number(e.target.value))} className="accent-amber-600" />
        </div>
        <span className="text-xs text-stone-500 ml-auto">{filtered.length} productos</span>
      </div>
              ...
              <tbody>
                ...
                {selItems[0]?.type==="machine" && (
                  <>
                    ...
                    <tr className="border-t border-stone-800"><td className="p-2 font-bold">Calentamiento</td>{selItems.map(s=> {
                      const v = selItems.map(x=>x.heat!); const best = Math.min(...v);
                      return <td key={s.id} className={`p-2 ${s.heat===best?"text-emerald-400 font-bold":""}`}>{s.heat! <60 ? `${s.heat}s` : `${Math.round(s.heat!/60)}min`}</td>;
                    })}</tr>
                  </>
                )}
